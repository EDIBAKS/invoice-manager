...
created(){
  //get current date
  //we dont want it to populate by default while the edit form loads
  if(!this.editInvoice){
    this.invoiceDateUnix=Date.now()
  this.invoiceDate=new Date(this.invoiceDateUnix).toLocaleDateString('en-us',this.dateOptions)
  }
 //populate the edit form with currently selected invoice details
if(this.editInvoice){
...
methods:{
  //all these mutations from our store help us to
  //open-close invoice modal
  //switch from create to edit modal
  //switch from one invoice to another
  ...mapMutations(["TOGGLE_INVOICE","TOGGLE_MODAL","TOGGLE_EDIT_INVOICE"]),
  //prompt on click away from invoice Modal
  ...mapActions(['UPDATE_INVOICE','GET_INVOICES']),
 // checkClick(e){
//if(e.target===this.$ref.invoiceWrap)
//this.TOGGLE_MODAL();
  //},
  //close the modal
  checkClick(e) {
...
this.invoiceItemList.forEach(item=>{
  ...
  alert('please enter invoice items')
}
this.loading=true;
...
this.calInvoiceTotal();
//const docRef = await addDoc(collection(db, "invoices"), {
  const updateRef = doc(db, "invoices", this.docId);
...
this.UPDATE_INVOICE(data);
    ...
    submitForm(){
      //the actual check between update and create is here
      if(this.editInvoice){
        this.updateInvoice();
        return;
      }
